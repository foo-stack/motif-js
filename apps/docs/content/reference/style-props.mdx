...
// ['_hover', '_focus', '_active', '_disabled', '_checked', '_selected'] as const`} />
...
native, the bags drive `Pressable`'s state callbacks. `_checked` / `_selected` cover the
interactive ARIA states form and composite-widget elements expose — so a styled toggle or tab can
react to its own checked / selected state in pure CSS, with no JS.
