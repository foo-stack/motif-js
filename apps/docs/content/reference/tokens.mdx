...
Two color groups carry intent. `action.{primary,danger,success}` is for solid controls —
`{ bg, fg, hover }`, where `fg` reads on the filled `bg`. `status.{neutral,info,success,warning,danger}`
