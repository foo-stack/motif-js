...
<ApiSignature
  name="applyRenameV3"
  status="stable"
  signature={`function applyRenameV3(source: string): { code: string; changed: boolean }`}
  params={[
    {
      name: 'source',
      type: 'string',
      required: true,
      description: 'File contents to transform. Returns the rewritten string and a flag indicating whether any rewrite occurred. The transform is whole-file: pass one file in, get one file out.',
    },
  ]}
/>
...
for (const file of files) {
  ...
  const { code } = applyRenameV3(source);
