...
<CodeBlock lang="ts" filename="theme/light.ts" code={`import { createTheme } from 'motif-js';
...
<CodeBlock lang="tsx" filename="components/Surface.tsx" code={`import { styled } from 'motif-js';
...
Mark `motif-js` as a peer dependency, not a regular one. Consumers install motif-js
