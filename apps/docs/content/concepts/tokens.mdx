---
title: Tokens
description: Tokens are the named values — colours, spacing, type sizes — that every motif style resolves through.
...
<CodeBlock lang="ts" code={`const tokens = {
  colors: {
    ink: '#1C1917',
    paper: '#FBF7F2',
...
<CodeBlock lang="ts" code={`'$colors.blue.500'; // → '#2563EB'
...
<CodeBlock lang="ts" code={`interface Theme {
  readonly name: string;
  readonly tokens: TokenMap;
}`} />

`createTheme` is a thin factory. It does not transform the input - it preserves the literal
token shape, which is what makes the paths derivable. `$`-references autocomplete against this
theme once you declare it; see [Token autocomplete](/guides/token-autocomplete) for the one-time
setup.
...
<Callout variant="info" title="Where to next">
  [Style props](/concepts/style-props) covers the prop catalog that consumes these tokens.
  [Theming](/concepts/theming) covers how a theme becomes active at runtime — `<ThemeProvider>`,
  the `data-theme` attribute, and switching themes without a flash.
</Callout>
